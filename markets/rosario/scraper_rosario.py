"""
scraper_rosario.py — Scraper for Drogaria Rosário (https://www.drogariarosario.com.br)

Platform  : VTEX IO (account `dmvfarma` — SAME catalog as drogasmil/farmalife/tamoio
            but its OWN price table/sales channel: prices differ ~11/12, so distinct)
...
Category note:
    fq filtering works. ~229 leaf categories, ~28k products (same dmvfarma catalog), NONE over the 2550
    cap (biggest leaf ~1415, verified 2026-08-25) -> plain Drogal-style leaf walk,
    no price-range subdivision needed.

Usage:
    python -m markets.rosario.scraper_rosario              # scrape -> DB
    python -m markets.rosario.scraper_rosario --limit 500  # test run -> DB
    python -m markets.rosario.scraper_rosario --csv        # scrape -> DB + CSV
"""
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Scrape Drogaria Rosario -> PostgreSQL (DB is always written; CSV is optional)"
    )
    ...
    from db.db_manager import RosarioDB, load_env
    load_env(args.env)
