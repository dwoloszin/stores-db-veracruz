"""
scraper_agillemed.py — Scraper for Agille Med (https://www.agillemed.com.br)

Platform   : Tray Commerce (Brazilian e-commerce platform).
Data source : Public Tray Store API
                https://www.agillemed.com.br/web_api/products?limit=50&page=N
...
EAN        : First-class API field ("ean"), ~98% coverage — no enrichment needed.
Prices     : "price" / "promotional_price" are decimal strings in BRL.
             promotional_price == "0" means no active promotion.

Usage:
    python -m markets.agillemed.scraper_agillemed              # scrape -> DB
    python -m markets.agillemed.scraper_agillemed --limit 200  # test run -> DB
    python -m markets.agillemed.scraper_agillemed --csv        # scrape -> DB + CSV
"""
...
BASE_URL  = "https://www.agillemed.com.br"
STORE_ID  = "agillemed"
...
def _fetch_page(
    ...
    total_count comes from paging.total (0 on error).
    """
    try:
        r = session.get(
            API_URL,
...
def _standardize(raw: Dict) -> Optional[Dict]:
    ...
    # Category hint from the slug (Tray slugs are "categoria/produto" when categorized).
    ...
    if isinstance(imgs, list) and imgs:
        ...
        if isinstance(first, dict):
            image_url = str(first.get("https") or first.get("http") or "").strip()
...
def scrape(db, limit: Optional[int] = None) -> Dict:
    ...
    while True:
        ...
        if batch:
            ...
            print(
                f"  page {page:>3}  got={len(products)}  saved={stats['upserted']}  "
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Scrape Agille Med (agillemed.com.br) -> PostgreSQL (DB always written; CSV optional)"
    )
    ...
    from db.db_manager import AgillemedDB, load_env
    load_env(args.env)
