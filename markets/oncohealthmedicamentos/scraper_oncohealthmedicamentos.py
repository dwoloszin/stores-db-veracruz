"""
scraper_oncohealthmedicamentos.py — Scraper for Onco Health Medicamentos
(https://www.oncohealthmedicamentos.com.br)

Platform  : Tray / CommerceSuite (Wake)
API       : /web_api/products  (public Tray Web API, JSON, no auth for reads)
Pagination: ?limit=50&page=N  (maxLimit 50); paging.total gives the count
Fields    : the Product object already includes everything we need:
                id                  -> product_id
                name                -> product_name
                ean                 -> ean / barcode (13-digit, ~100% coverage)
                price               -> regular price
                promotional_price   -> promo price ("0" when none)
                available           -> is_available ("1"/"0")
                brand               -> brand
                url.https / slug    -> product_url + category_path
                ProductImage[0]     -> image_url
EAN       : first-class field; no enrichment step needed.

NOTE: product pages are behind a JS shell / bot protection, so HTML/JSON-LD
scraping is unreliable — the Web API is the stable source and also exposes
regular vs promotional price directly.

Usage:
    python -m markets.oncohealthmedicamentos.scraper_oncohealthmedicamentos              # scrape -> DB
    python -m markets.oncohealthmedicamentos.scraper_oncohealthmedicamentos --limit 100  # test run -> DB
    python -m markets.oncohealthmedicamentos.scraper_oncohealthmedicamentos --csv        # scrape -> DB + CSV
"""
...
BASE_URL   = "https://www.oncohealthmedicamentos.com.br"
...
def _image_url(p: Dict) -> str:
    ...
    if isinstance(imgs, list) and imgs:
        ...
        if isinstance(first, dict):
            return str(first.get("https") or first.get("http") or first.get("thumbs") or "").strip()
    return ""
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Scrape Onco Health Medicamentos -> PostgreSQL (DB always written; CSV optional)"
    )
    ...
    from db.db_manager import OncoHealthMedicamentosDB, load_env
    load_env(args.env)

    db    = OncoHealthMedicamentosDB()
