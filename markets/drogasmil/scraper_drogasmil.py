"""
scraper_drogasmil.py — Scraper for Drogasmil (https://www.drogasmil.com.br)

Platform  : VTEX IO (account `dmvfarma`, Grupo DMV / Rio de Janeiro)
...
Category note:
    fq filtering works. ~229 leaf categories, ~28k products, NONE over the 2550
    cap (biggest leaf ~1415, verified 2026-08-25) -> plain Drogal-style leaf walk,
    no price-range subdivision needed.

Usage:
    python -m markets.drogasmil.scraper_drogasmil              # scrape -> DB
    python -m markets.drogasmil.scraper_drogasmil --limit 500  # test run -> DB
    python -m markets.drogasmil.scraper_drogasmil --csv        # scrape -> DB + CSV
"""
...
BASE_URL  = "https://www.drogasmil.com.br"
STORE_ID  = "drogasmil"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Scrape Drogasmil -> PostgreSQL (DB is always written; CSV is optional)"
    )
    ...
    from db.db_manager import DrogasmilDB, load_env
    load_env(args.env)
