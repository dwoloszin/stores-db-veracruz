"""
enrich_ean_eualiria.py — Backfill EAN/barcode for Alíria (eualiria.com.br) products.

The WooCommerce Store API does not expose EAN as a field, but every product page
(rendered by the `ean-for-woocommerce` plugin) carries it in two reliable places:
    - JSON-LD structured data:  "gtin13":"7896261020764"
    - Plugin markup:            <span class="ean">7896261020764</span>

This backfill fetches each product page missing an EAN and writes the value back.

Usage:
    python -m markets.eualiria.enrich_ean_eualiria
    python -m markets.eualiria.enrich_ean_eualiria --limit 100
    python -m markets.eualiria.enrich_ean_eualiria --workers 12
"""
...
# gtin13 in JSON-LD is the most reliable; the plugin <span class="ean"> is the fallback.
_GTIN_RE = re.compile(r'"gtin(?:8|12|13|14)?"\s*:\s*"(\d{8,14})"')
_SPAN_RE = re.compile(r'<span class="ean">\s*(\d{8,14})')
...
def _fetch_ean(session: requests.Session, product_id: str, url: str) -> Tuple[str, Optional[str]]:
    for attempt in range(3):
        try:
            ...
            html = r.text
            m = _GTIN_RE.search(html) or _SPAN_RE.search(html)
            if m:
                ean = m.group(1)
...
def enrich(workers: int = 12, limit: Optional[int] = None, db=None) -> Dict[str, int]:
    from db.db_manager import EualiriaDB
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Backfill EAN for Alíria (eualiria.com.br) products")
