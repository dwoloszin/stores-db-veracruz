"""
scraper_alianza.py — Scraper for Alianza Pharma (https://www.alianzapharma.com.br)

Platform  : Tray / CommerceSuite (Wake) — images.tcdn.com.br
API       : /web_api/products  (public Tray Web API, JSON, no auth for reads)
Pagination: ?limit=50&page=N  (maxLimit 50); paging.total gives the count (~453)
Fields    : id, name, ean, price, promotional_price, available, brand,
            url.https / slug, ProductImage[0].
EAN       : first-class field; no enrichment step needed.

Note: alianzapharma.com.br and oncovitonline.com.br are the same Tray backend
(identical catalogue/prices) — only one is tracked.

Usage:
    python -m markets.alianza.scraper_alianza              # scrape -> DB
    python -m markets.alianza.scraper_alianza --limit 100  # test run -> DB
    python -m markets.alianza.scraper_alianza --csv        # scrape -> DB + CSV
"""
...
BASE_URL   = "https://www.alianzapharma.com.br"
STORE_ID   = "alianza"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Scrape Alianza Pharma -> PostgreSQL (DB always written; CSV optional)"
    )
    ...
    from db.db_manager import AlianzaDB, load_env
    load_env(args.env)
